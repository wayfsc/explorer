    ...
    is_locked("db_index", function (exists) {
      if (exists) {
        ...
        create_lock("db_index", function () {
          ...
          let cnt = Math.floor(blocks_to_scan.length / 100);
          console.log('chunk size', cnt);
          let prom = Promise.resolve();

          for (let i = 0; i < cnt; i += 100) {

            let chunk = blocks_to_scan.slice(i, i + 100);

            prom = prom.then(() => {
              return new Promise((resolve, reject) => {

                let promises = [];
                for (let block_height of chunk) {
                      ...
                      lib.get_block(blockhash, function (block) {
                        ...
                        for (let txid of block.tx) {
                          txpromise = txpromise.then(() => {
                            return new Promise(txres => {

                              Tx.findOne({ txid: txid }, function (err, tx) {

                                if (!tx) {
                                  save_tx(txid, block_height, function (err) {
                                    console.log('save tx', !!err)
